...
import net.jeeshop.core.framework.oscache.ManageCache;
import net.jeeshop.core.model.Menu;
import net.jeeshop.core.model.MenuItem;
...
//	}
	...
	public String login(HttpSession session,@ModelAttribute("e") User e, ModelMap model) throws Exception {
		...
		if (u == null) {
			errorMsg = "登录失败，账户或密码错误！";
			logger.error("登录失败，账户或密码错误,{}", e.getUsername());
...
//			}
	...
	private void loginLog(User u,String log) {
		...
		if(!systemlog.getLoginIP().equals("127.0.0.1") && !systemlog.getLoginIP().equals("localhost")){
			...
			//异地登录的判断方法为：先比较本次登录和上次登录的区域位置，如果不一致，说明是异地登录；如果获取不到区域，则比较IP地址，如果IP地址和上次的不一致，则是异地登录
...
//		errorMsg = "旧密码输入错误，修改密码失败!";
	...
	@Override
	protected void selectListAfter(PagerModel pager, User e) {
...
//		}
	...
	public String initManageIndex(){
		//店主每次登录后台都需要加载综合统计数据？！还是说每次都触发加载，但是到底加载不加载具体看系统的加载策略？！
