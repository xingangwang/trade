package net.jeeshop.core.framework.oscache;
...
//		SystemManager.attrs = attrs;
        ...
        if(attrs!=null && attrs.size()>0){
			...
			for(int i=0;i<attrs.size();i++){
				...
				if(Integer.valueOf(mainAttr.getPid())==0){
...
//			Catalog catalog = SystemManager.catalogsMap.get(catalogID);
	...
	public static List<Attribute> loadAttrByCatalogID(int catalogID) {
		...
		for (int i = 0; i < attrs.size(); i++) {
			...
			if (Integer.valueOf(attr.getPid()) == -1) {
				continue;
			}
			...
			if (attr.getCatalogID() == catalogID) {
				...
				for (int j = 0; j < attrs.size(); j++) {
					...
					if (Integer.valueOf(attr2.getPid()) == id) {
		...
		if(list!=null){
			for(int i=0;i<list.size();i++){
				...
				if(!activity.isExpire()){
					//计算活动多久结束，是否已结束
					activity.setActivityEndDateTime(DateUtil.getActivityEndDateTimeString(activity.getEndDate()));
				}

				//折扣类型 ，则进行折扣计算
				if(activity.getDiscountType().equals(Activity.activity_discountType_d)){
					activity.setDiscountFormat(String.valueOf(Double.valueOf(activity.getDiscount())));
				}
