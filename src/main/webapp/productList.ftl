...
function defaultProductImg(){ 
	...
	<div class="container" style="width:1210px;">
		...
		<div class="row" style="padding:0;">
			<div class="col-xs-3" style="padding:0; width:230px;">
				<#include "/catalog_superMenu.ftl">
				</br>
				<#--<#include "/productlist_left_picScroll.ftl">-->
			</div>
			<div class="col-xs-9" style="width:980px;margin-top:15px;">
				<!-- 导航栏 -->
				<div class="row">
				...
				<div class="row" style="margin: 10px 0px;">
					<div class="col-xs-12">
						<#if catalogChildren?? && (catalogChildren?size > 0)>
							<div>
						...
						<div class="col-xs-12">
							<span class="attr_css" style="margin:5px;font-weight: bold;">排序</span>
								<span class="label ${(e.orderBy??&&e.orderBy==1)?string('label-success','')} attr_css" style="margin:5px;">
									<a href="${basepath}/catalog/${catalogCode!""}.html?orderBy=1&attrID=${e.attrID!""}&key=${key!""}">热门</a>
								</span>

								<span class="label ${(e.orderBy??&&e.orderBy==2)?string('label-success','')}  attr_css" style="margin:5px;">
									<a href="${basepath}/catalog/${catalogCode!""}.html?orderBy=2&attrID=${e.attrID!""}&key=${key!""}">价格</a>
								</span>

								<span class="label ${(e.orderBy??&&e.orderBy==3)?string('label-success','')}  attr_css" style="margin:5px;">
									<a href="${basepath}/catalog/${catalogCode!""}.html?orderBy=3&attrID=${e.attrID!""}&key=${key!""}">最新</a>
								</span>
						</div>
					</div>
