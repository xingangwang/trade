	...
	<input type="hidden" value="${e.id!""}" name="id"/>
		...
		<div id="updateMsg"><font color='red'>${e.updateMsg!""}</font></div>
				<#if e.status??&&e.status=="INIT">
					<a href="updateOrderStatus?id=${e.id!""}&status=PASSED" onclick="return onSubmit(this);"
						value="已审核" class="btn btn-primary">审核</a>
				<#elseif e.status=="PASSED">
					...
					<a href="updateOrderStatus?id=${e.id!""}&status=CANCELLED" onclick="return onSubmit(this);"
						value="取消订单" class="btn btn-danger">取消</a>
				<#elseif e.status??&&e.status=="PAID">
					<a href="updateOrderStatus?id=${e.id!""}&status=SENT" onclick="return onSubmit(this);"
						value="已发货" class="btn btn-primary">发货</a
				<#elseif e.status??&&e.status=="SENT">
					<a onclick="return onSubmit(this);"
						value="已发货" class="btn" disabled="disabled">待收货</a>
				<#elseif e.status??&&e.status=="SIGNED">
					<a href="updateOrderStatus?id=${e.id!""}&status=FILED" onclick="return onSubmit(this);"
						value="已归档" class="btn btn-primary">归档</a>
				<#else>
					<a value="已归档" class="btn" disabled="disabled">已归档</a>
				</#if>
		</div>
	...
	<div id="addPayDiv" style="display: none;">
		<table class="table">
			...
			<tr>
				<td>支付金额</td>
				<td>
					<input name="orderpay.payamount" value="${(e.orderpay.payamount)!""}" data-rule="支付金额:required;orderpay.payamount;" >
				</td>
			</tr>
			<tr>
				<td>备注</td>
			...
			<tr>
				<td colspan="2">
					<button type="submit" method="insertOrderpay" onclick="return onSubmit(this);" value="确认" class="btn btn-primary">确认</button>
					<button id="cancelPayBtn" type="button" value="取消" class="btn" onclick="cancelSubmit('orderpay.payamount', '${(e.orderpay.payamount)!""}');">取消</button>
				</td>
			</tr>
		</table>
	</div>
	...
	<div id="updatePayMoneryDiv" style="display: none;">
		<table class="table">
			...
			<tr>
				<td>订单总金额</td>
				<td>
					<input name="amount" value="${e.amount!""}" data-rule="订单总金额:required;amount;" >
				</td>
			</tr>
			<tr>
				<td>备注</td>
			...
			<tr>
				<td colspan="2">
					<button type="submit" method="updatePayMonery" onclick="return onSubmit(this);" value="确认" class="btn btn-primary">确认</button>
					<button id="cancelUpdatePayMoneryBtn" type="button" value="取消" class="btn" onclick="cancelSubmit('amount', '${e.amount!""}');">取消</button>
				</td>
			</tr>
		</table>
	</div>
...
<!-- 					<th>订单备注</th> -->
				...
				<tr>
					...
					<td>&nbsp;${e.createdate!""}</td>
					<td>
						<#assign map = {'':'','INIT':'未审核','PASSED':'已审核','PAID':'已付款','SENT':'待收货','SIGNED':'已签收','CANCELLED':'已取消','FILED':'已归档'}>
						<span class="badge badge-important">${map[e.status]!e.status}</span>
					</td>
					<td>
						<#assign payStatus = {'n':'未支付','y':'全部支付','p':'部分支付'}>
...
function onSubmit(obj){
	if ($("input[name='orderpay.payamount']").val() === '') {
		alert('支付金额不能为空');
		return false;
	} 
	...
	//if(confirm("确认本次操作?")){
		//document.form1.action = $(obj).attr("method");
...
function cancelSubmit(inputName, value){
