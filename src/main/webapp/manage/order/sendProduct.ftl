...
<html>
<head>
<#include "/resource/common_html_meat.ftl">
<base href="${basepath}/">
	        ...
	        $.ajax({
	            ...
	            success: function(d){
	            	if(d.success){
	            		alert("发货成功！");
	            		//页面专项明细页面
	            		window.location.href = "manage/order/toEdit?id="+_orderid;
	            	}else{
	            		var errorStr = d.message;
...
<body>
	<form id="form" name="form" method="post" theme="simple" >
		<input type="hidden" name="WIDtrade_no" value="${e.tradeNo}"/>
		<input type="hidden" id="orderid" name="orderid" value="${e.id}"/>
		...
		<table class="table table-bordered">
			<tr>
				...
				<td nowrap="nowrap">
					...
					<#assign map = {'EXPRESS':'快递','POST':'平邮','no':'不需要物流 '}>
					<select name="WIDtransport_type" id="WIDtransport_type" class="search-query input-medium"
			...
			<tr id="companyTR">
				...
				<th nowrap="nowrap">
					...
					<select name="expressCompanyName" id="expressCompanyName">
						<#assign map = systemManager().getManageExpressMap() >
						<#list map?keys as key>
							<option value="${key}">${map[key]}</option>
						</#list>
					</select>
			...
			<tr>
				...
				<th nowrap="nowrap">
					<input name="WIDinvoice_no" id="WIDinvoice_no" data-rule="运单号:required;WIDinvoice_no;length[1~20];" maxlength="20" size="20"/>
				</th>
			</tr>
			<tr>
