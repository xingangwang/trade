...
<#function getTitle >
	<#local title="" />
	<#if e.refundStatus??>
		<#if e.refundStatus == 'WAIT_SELLER_AGREE'>
			<#local title="退款管理"/>
		<#elseif e.refundStatus == 'WAIT_SELLER_CONFIRM_GOODS'>
			<#local title="退货管理"/>
		</#if>
	<#else>
		<#local title="订单管理"/>
...
<@page.pageBase currentMenu=getTitle()>
<style type="text/css">
.titleCss {
	...
	<form action="${basepath}/manage/order" method="post" theme="simple">
		<table class="table table-bordered">
			<tr>
				...
				<td>订单状态</td>
				<td>
					<#assign orderStatus = {'':'','INIT':'待审核','PASSED':'待付款','PAID':'待发货','SENT':'待收货','SIGNED':'已签收','CANCELLED':'已取消','FILED':'已归档'}>
                    <select id="status" name="status" class="search-query input-medium">
						<#list orderStatus?keys as key>
                            <option value="${key}" <#if e.status?? && e.status==key>selected="selected" </#if>>${orderStatus[key]}</option>
						</#list>
                    </select></td>
				<td>支付状态</td>
				<td>
					<#assign payStatus = {'':'','n':'未支付','y':'完全支付'}>
                    <select id="paystatus" name="paystatus" class="search-query input-medium">
						<#list payStatus?keys as key>
                            <option value="${key}" <#if e.paystatus?? && e.paystatus==key>selected="selected" </#if>>${payStatus[key]}</option>
						</#list>
                    </select>
					</td>
				<td>用户账号</td>
			...
			<tr>
				<td>退款状态</td>
				<td colspan="21">
					<#assign refundStatus = {'':'','WAIT_SELLER_AGREE':'等待卖家同意退款','WAIT_BUYER_RETURN_GOODS':'卖家同意退款，等待买家退货','WAIT_SELLER_CONFIRM_GOODS':'买家已退货，等待卖家收到退货','REFUND_SUCCESS':'退款成功，交易关闭'}>
                    <select id="refundStatus" name="refundStatus" class="search-query input-medium">
						<#list refundStatus?keys as key>
                            <option value="${key}" <#if e.refundStatus?? && e.refundStatus==key>selected="selected" </#if>>${refundStatus[key]}</option>
						</#list>
                    </select></td>
			</tr>
			<tr>
